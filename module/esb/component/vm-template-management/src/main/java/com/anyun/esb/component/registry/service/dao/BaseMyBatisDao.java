...
public class BaseMyBatisDao {
    ...
    public <T>Page<T> selectPageList(Class<T> type, String sql, Object params, int pageNumber, int pageSize) {
        SqlSession session = databaseFactory.getSession();
        Page<T> page = null;
        try {
            page = PageHelper.startPage(pageNumber,pageSize,true);
            session.selectList(sql, params, new RowBounds(pageNumber, pageSize));
        } catch (Exception e) {
            throw new DaoException(0x00000021, e);
        } finally {
            session.close();
