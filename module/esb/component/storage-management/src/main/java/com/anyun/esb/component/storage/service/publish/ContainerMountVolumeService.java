...
import com.anyun.cloud.param.ContainerMountVolumeParam;
 ...
 */
public class ContainerMountVolumeService extends AbstractBusinessService {
    private static final Logger LOGGER = LoggerFactory.getLogger(ContainerMountVolumeService.class);
    private VolumeService volumeService;
    private VolumeDao volumeDao;
    private StorageDao storageDao;

    public ContainerMountVolumeService() {
    ...
    public String getName() {
        return "container_mount_volume";
    }

    @Override
    public String getDescription() {
        return "ContainerMountVolumeService";
    }
    ...
    public Object process(Endpoint endpoint, Exchange exchange) throws JbiComponentException {
        ...
        ContainerMountVolumeParam param = JsonUtil.fromJson(ContainerMountVolumeParam.class, postBody);
        if (param == null) {
        ...
        if (StringUtils.isEmpty(param.getMountPath())) {
            LOGGER.debug("param [mountPath] is empty");
            JbiComponentException exception = new JbiComponentException(2000, 1000);
            exception.setMessage("param [mountPath] is empty");
            exception.setUserMessage("服务参数格式化错误[" + exception + "]");
            exception.setUserTitle("参数验证失败");
            throw exception;
        }
        ...
        try {
            ...
            JbiComponentException exception = new JbiComponentException(2000, 1000);
            exception.setMessage("Container mount volume  error [" + e.getMessage() + "]");
            exception.setUserMessage("容器挂载卷失败[" + e.getMessage() + "]");
            exception.setUserTitle("容器挂载卷失败");
            throw exception;
        }
    }
}
