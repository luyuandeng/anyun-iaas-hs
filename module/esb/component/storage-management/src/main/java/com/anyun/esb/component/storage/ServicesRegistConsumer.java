...
public class ServicesRegistConsumer extends ConsumerThread {
    ...
    private String componentName = "anyun-storage";
    ...
    private void registComponentServices(Jedis jedis, String componentId) throws Exception {
        ...
        for (Class<? extends BusinessService> aClass : serviceClasses) {
            try {
                ...
                LOGGER.info("Regist AnyunCloud storage component service to znode [{}-{}]", businessService.getClass().getName(), serviceId);
            } catch (Exception ex) {
                LOGGER.warn("Storage service class [{}] regist error [{}]", aClass.getName(), ex.getMessage());
            }
        }
    }
